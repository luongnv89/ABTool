...
}class BPrimeParser extends Parser;

options {
	importVocab= B;
	exportVocab= BPrimeLexer;
	k= 10 ;
...
protected paramName :B_IDENTIFIER 
        (
            B_LPAREN^
            listTypedIdentifier
            B_RPAREN!
...
protected paramRename :nameRenamed 
        (
            B_LPAREN^
            listTypedIdentifier
            B_RPAREN!
...
protected listTypedIdentifier :typedIdentifier ( B_COMMA^ typedIdentifier  )*
    ;

protected typedIdentifier :nameRenamed (B_INSET^ basic_sets)?
    ;

protected sets_declaration :set_declaration 
        ( 
            (	B_SEMICOLON^ 
            | 	B_COMMA^
            ) 
...
protected operationHeader :paramName
    |	listTypedIdentifier B_OUT^ paramName
    ;

protected subst_mch :identite
...
protected substitution_block_mch {
    boolean i = false;
}
:rr:"BEGIN"^
            substitution_mch
...
protected substitution_block_ref { 
    boolean i = false;
}
:rr:"BEGIN"^
            parallele_ref
...
// Our Packages
	import ABTOOLS.DEBUGGING.*;
    	import ABTOOLS.ANTLR_TOOLS.*;
}class BPrimeLexer extends Lexer;
